...
contract MyEscrow is IEscrow, ReentrancyGuard {
    ...
    function confirmReceipt() external onlyBuyer inState(State.Created) {
        ...
        if (tokenBalance != 0) {
            i_tokenContract.safeTransfer(i_buyer, tokenBalance); // send back any extra tokens
        }
    }
    ...
    function resolveDispute(uint256 buyerAward) external onlyArbiter nonReentrant inState(State.Disputed) {
        ...
        uint256 amountToSeller = i_price - i_arbiterFee - buyerAward;
        ...
        if (tokenBalance != 0) {
            i_tokenContract.safeTransfer(i_seller, amountToSeller);
        }

        // send back any extra token to seller or arbiter or any other address over here
        // here we are sending it back to the buyer
        tokenBalance = i_tokenContract.balanceOf(address(this));
        if (tokenBalance != 0) {
